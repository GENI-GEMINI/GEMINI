...
perfSONAR_PS::Services::MP::Skeleton - A module that provides methods for the 
perfSONAR-PS Skeleton MP.
...
sub makeMeasurement {
    ...
    my $metadatadb_file = $self->{CONF}->{"skeleton"}->{"metadata_db_file"};
    my $metadatadb_type = $self->{CONF}->{"skeleton"}->{"metadata_db_type"};
    ...
    if ( $metadatadb_type eq "file" ) {
        ...
        my $error;
        my $metadatadb = perfSONAR_PS::DB::File->new( { file => $metadatadb_file } );
        $metadatadb->openDB( { error => \$error } );
        unless ( $metadatadb ) {
            my $msg = "Couldn't initialize store file: $error";
            $self->{LOGGER}->error( $msg );
            ${ $parameters->{error} } = $msg if ( $parameters->{error} );
        ...
        my $results = $metadatadb->querySet( { query => "/nmwg:store/nmwg:metadata" } );
        if ( $results->size() > 0 ) {
            foreach my $metadata ( $results->get_nodelist ) {
                if ( $metadata->getAttribute( "id" ) ) {
                    my $data = $metadatadb->querySet( { query => "/nmwg:store/nmwg:data[\@metadataIdRef=\"" . $metadata->getAttribute( "id" ) . "\"]" } );
                    my $hostName  = extract( find( $metadata,            ".//unis:node/unis:address[\@type=\"hostname\"]",                                      1 ), 0 );
                    my $eventType = extract( find( $metadata,            ".//nmwg:eventType",                                              1 ), 0 );
                    my $command   = extract( find( $data->get_node( 1 ), ".//nmwg:key/nmwg:parameters/nmwg:parameter[\@name=\"command\"]", 1 ), 0 );
                    open( COMMAND, $command . " |" );
                    my @res = <COMMAND>;
                    close( COMMAND );
                    $measurements->{$hostName}->{$eventType} = $res[0];
                    $self->{LOGGER}->debug( "Performing \"" . $command . "\" on " . $hostName );
                }
            }

            my ( $fileHandle, $fileName ) = tempfile();
            my $frozen = nfreeze( $measurements );
            my $coded = unpack( "H*", $frozen ) . "\n";
            print $fileHandle $coded;
            close( $fileHandle );
            system( "mv " . $fileName . " " . $self->{CONF}->{"skeleton"}->{"data_file"} );
        }
        else {
...
sub handleData {
    ...
    my $valueUnits = extract( find( $parameters->{data}, "./nmwg:key//nmwg:parameter[\@name=\"valueUnits\"]", 1 ), 0 );

    my $hostName = extract( find( $parameters->{metadata}, ".//unis:node/unis:address[\@type=\"hostname\"]", 1 ), 0 );
    my $eventType = extract( find( $parameters->{metadata}, ".//nmwg:eventType", 1 ), 0 );

    open( DATA, $self->{CONF}->{"skeleton"}->{"data_file"} );
    my @result = <DATA>;
    close( DATA );
    my $frozen = pack "H*", $result[0];
    my $value = thaw( $frozen );

    startData( $parameters->{output}, $id, $parameters->{id}, undef );

    my %attrs = ();
    if ( $timeType eq "iso" ) {
        my ( $sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst ) = gmtime( $sec );
        $attrs{"timeType"} = "ISO";
    ...
    $attrs{"value"}      = $value->{$hostName}->{$eventType};
