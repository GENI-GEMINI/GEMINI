...
# All rights reserved.
#
package ProtoGENI::GeniCredential;
...
use ProtoGENI::GeniCertificate;
use ProtoGENI::GeniUtil;
use ProtoGENI::GeniXML;
...
sub CreateSigned($$$;$)
{
    ...
    my $credential = ProtoGENI::GeniCredential->Create($target, $owner);
...
sub CreateFromSigned($$;$)
{
    ...
    my ($extensions) = ProtoGENI::GeniXML::FindNodes('//n:extensions', 
                        $root)->get_nodelist;
	...
	ProtoGENI::GeniCertificate->LoadFromString($cert_node->to_literal());
    return undef
    ...
    if (!ProtoGENI::GeniHRN::IsValid($target_certificate->urn())) {
    ...
    if (!ProtoGENI::GeniHRN::IsValid($owner_certificate->urn())) {
    ...
    foreach my $cap (ProtoGENI::GeniXML::FindNodes('.//n:privileges/n:privilege',
					 $root)->get_nodelist()) {
	my $name = ProtoGENI::GeniXML::FindElement('n:name', $cap);
	my $delegate = ProtoGENI::GeniXML::FindElement('n:can_delegate', $cap);
...
sub IsSliceCredential($) {
    ...
    my ($authority, $type, $id) = ProtoGENI::GeniHRN::Parse($self->target_urn());
...
sub CheckCredential($)
{
    ...
    my $credential = ProtoGENI::GeniCredential->CreateFromSigned($credstr);
    if (!defined($credential)) {
	return "Could not create credential object";
    }
    #
    # Well formed credentials must now have URNs.
    #
    return "Malformed credentials; missing URNs"
	if (! (defined($credential->owner_urn()) &&
	       defined($credential->target_urn()) &&
	       ProtoGENI::GeniHRN::IsValid($credential->owner_urn()) &&
	       ProtoGENI::GeniHRN::IsValid($credential->target_urn())));
	
    #
    # Make sure the credential was issued to the caller.
    #
    if ($credential->owner_urn() ne $ENV{'GENIURN'}) {
	return "This is not your credential";
    }
