...
#
# XXXXXX XXX: SIGNATURE CHANGED XXXXXXXXXXXX
#
sub Create($$$$$$;$$)
{
    my ($class, $what, $days, $urn, $hrn, $email, $uuid, $url) = @_;
    # Let mkcert generate a new one.
    $uuid = ""
	if (!defined($uuid));
    $url  = (defined($url) ? "-u $url" : "");

    if (! open(CERT, "$MKCERT -i \"$urn\" -t $days $hrn $uuid |")) {
    ...
    $certificate->{'CERT'}->{'string'} = join('', @certlines);
